...
func NewUser(name, lastName, username, email, passwordHash string, roleID uuid.UUID) *User {
	return &User{
		ID:           uuid.New(),
		Name:         name,
		LastName:     lastName,
...
func (u *User) Update(name, lastname, user, email, phone string, dni int, roleID uuid.UUID) {
	u.Name = name
	u.LastName = lastname
	u.Username = user
	u.Email = email
